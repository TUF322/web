import React from 'react';
import './Projects.css';
import { useLanguage } from '../context/LanguageContext';

const GithubIcon = ({ size = 20 }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="currentColor">
    <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
  </svg>
);

const ExternalIcon = () => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
    <path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"></path>
    <polyline points="15 3 21 3 21 9"></polyline>
    <line x1="10" y1="14" x2="21" y2="3"></line>
  </svg>
);

const Projects = () => {
  const { t } = useLanguage();

  const projects = [
    {
      key: 'concier',
      technologies: ['Flutter', 'Dart', 'Django', 'REST APIs', 'AWS', 'Figma'],
      link: null,
      github: null,
      featured: true
    },
    {
      key: 'domu',
      technologies: ['Laravel', 'SQLite', 'Ionic', 'Angular', 'Capacitor'],
      link: null,
      github: null,
      inDevelopment: true,
      collaborators: [{ name: 'Luis Miguel', url: 'https://github.com/umiguel87' }]
    },
    {
      key: 'audio',
      technologies: ['React', 'WaveSurfer.js', 'JavaScript', 'UI/UX'],
      link: null,
      github: 'https://github.com/TUF322/audio_annotation'
    },
    {
      key: 'attenda',
      technologies: ['React', 'Node.js', 'Express', 'SQLite', 'JWT'],
      link: null,
      github: 'https://github.com/TUF322/Attenda'
    },
    {
      key: 'utask',
      technologies: ['Ionic', 'Angular', 'TypeScript', 'SQLite', 'Capacitor'],
      link: null,
      github: 'https://github.com/TUF322/UTask'
    },
    {
      key: 'internship',
      technologies: ['React', 'WaveSurfer.js', 'JavaScript'],
      link: null,
      github: 'https://github.com/TUF322/Dario_Intership'
    }
  ];

  return (
    <section id="projects" className="projects">
      <div className="projects-container">
        <div className="section-header">
          <h2 className="section-title">{t.projects.title}</h2>
          <div className="title-underline"></div>
          <p className="section-subtitle">{t.projects.subtitle}</p>
        </div>
        <div className="projects-grid">
          {projects.map((project, index) => {
            const item = t.projects.items[project.key];
            return (
              <div
                key={project.key}
                className={`project-card ${project.featured ? 'featured' : ''}`}
              >
                <div className="project-index">
                  [{String(index + 1).padStart(2, '0')}]
                  {project.featured && (
                    <span className="featured-tag">{t.projects.featured}</span>
                  )}
                </div>
                <div className="project-header">
                  <h3 className="project-title">{item.title}</h3>
                  <div className="project-links">
                    {project.link && (
                      <a
                        href={project.link}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="project-link"
                        aria-label={t.projects.viewProject}
                      >
                        <ExternalIcon />
                      </a>
                    )}
                    {project.github && (
                      <a
                        href={project.github}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="project-link"
                        aria-label={t.projects.viewCode}
                      >
                        <GithubIcon />
                      </a>
                    )}
                  </div>
                </div>
                {project.inDevelopment && (
                  <div className="project-status">{t.projects.inDevelopment}</div>
                )}
                <p className="project-description">{item.description}</p>
                {item.highlights && (
                  <div className="project-apps">
                    {item.highlights.map((highlight) => (
                      <span key={highlight} className="app-chip">
                        {highlight}
                      </span>
                    ))}
                  </div>
                )}
                {project.collaborators && (
                  <div className="project-collaborators">
                    <div className="collaborators-title">{t.projects.collaborators}</div>
                    <div className="collaborators-list">
                      {project.collaborators.map((collab) => (
                        <a
                          key={collab.url}
                          href={collab.url}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="collaborator-item"
                        >
                          <span className="collaborator-name">{collab.name}</span>
                          <span className="collaborator-icon">
                            <GithubIcon size={16} />
                          </span>
                        </a>
                      ))}
                    </div>
                  </div>
                )}
                <div className="project-technologies">
                  {project.technologies.map((tech) => (
                    <span key={tech} className="tech-tag">
                      {tech}
                    </span>
                  ))}
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
};

export default Projects;
